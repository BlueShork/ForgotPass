			...
			<div class="main" id="menu">
				...
				<?php include('./class/get_mobile_menu.php'); ?>
					<ul class="menu none1">
						...
						<li class="fonts left racc pseudo_li hover active"><a style="cursor:pointer;"><i class="fas fa-money-check-alt"></i> Gagner de l'argent</a>
							<ul class="submenue">
...
?>
			...
			<form method="POST" action="envoye_message.php?red=cashlinks.php" class="chat_form">
...
<div class="all-az row">
				<h2>Les Cashlinks<br><span class="regles">Comment ça marche ?</span></h2>
				<div class="all_contener">
			<?php 
	...
	while($donnees = $req->fetch()){
		...
		$img = $donnees['ban'];
		?>
		<div class="card partner ptc-card col-lg-3 pc_card">
					<!--<img src="images/sweet.png" alt="Erreur -59">-->
					<p class="nome "><?php echo $nom;?></p>
					<p class="gbucks 	">+ <?php echo $donnees['remise'];?></p>
					<img src="<?php echo $img;?>" class="banner 	">
					<p class="">Suivez simplement le lien, vous devez patienter et une fois fini notre site fait une redirection pour vous créditer vos points sur votre compte instantanément.</p>
					<!--<p class="name">+ <span class="chiffre"><?php echo $donnees['gains']; ?> <i class="fas fa-bullseye"></i></span></p>-->
			...
			$requete = $bdd->prepare('SELECT COUNT(*) AS clicks_journalier FROM quetes_journalier WHERE pseudo = :pseudo AND nom_quetes = :nom_quetes');
	$requete->execute(array(
	':pseudo' => $_SESSION['pseudo'],
	':nom_quetes' => $nom,
	));
	while($donneess = $requete->fetch()){
		if($donneess['clicks_journalier'] >= $limite and isset($_COOKIE[$id.'quetes'])){
			?>
			<div class="valider-check "><a class="participer"><i class="fas fa-check"></i> Vous avez terminé revenez demain !</a></div>
			<?php
		}
		elseif($donneess['clicks_journalier'] >= $limite and !isset($_COOKIE[$id.'quetes'])){
			?>
			<div class="valider-check"><a class="participer"><i class="fas fa-check"></i> Vous avez terminé revenez demain !</a></div>
		...
		elseif(empty($donneess['clicks_journalier']) and isset($_COOKIE[$id.'quetes'])){
			?>
			<div class="valider-checke "><a class="participer"><i class="fas fa-check"></i> Revenez à la même heure qu'hier ...</a></div>
			<?php
		}
		else{
			?>
			<div class="but"><a href="<?php echo $liens;?>" class="participer">Je réalise la mission</a></div>
			<?php
		}
	}
			...
			<div class="card partner ptc-card mobile_card">
					<!--<img src="images/sweet.png" alt="Erreur -59">-->
					<p class="nome "><?php echo $nom;?></p>
					<p class="gbucks 	">+ <?php echo $donnees['remise'];?></p>
					<img src="<?php echo $img;?>" class="banner 	">
