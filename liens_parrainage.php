...
if(isset($_SESSION['id']) AND isset($_SESSION['pseudo'])){
		...
		<html>
			<head>
				<title><?php site();?> - Parrainage</title>
				<meta charset="utf-8">
...
xhr_object.send(null);
				...
				<div class="presentation">
					<h1>Votre lien de parrainage :</h1>
						<p>Utiliser votre lien de parrainage pour le partager à vos amis et gagnez 10% des gains de vos filleuls. Pour gagner ces points votre filleul doit réaliser des missions.</p><br>
						<p class="links">Lien ptp :   http://<?php site(); ?>/inscription.php?parrain=<?php echo $_SESSION['pseudo']; ?></a></p>
						
						<?php 
						
						
						// Count how many points user has been win
						$req_p = $bdd->prepare('SELECT SUBSTRING(sum(gains), 1, 7) AS gains_total FROM quetes_liens WHERE nom_quetes = :nom_quetes AND pseudo = :pseudo');
						$req_p->execute(array(
						...
						if($p_point['gains_total'] == 0){
							$parrain_points=0;
						}else{
							$parrain_points = $p_point['gains_total'];
						}
						...
						<p>Vous avez gagné : <?php echo $parrain_points;?> </p>
					...
					<table class="table table-striped">
						...
						<tbody>
							...
							$req = $bdd->prepare('SELECT * FROM inscription WHERE parrain = :parrain');
							$req->execute(array(
								':parrain' => $_SESSION['pseudo'],
							));
							$i = 0;
							while($donnees = $req->fetch()){

								// get info from the credential user

								$req_user = $bdd->prepare('SELECT * FROM inscription WHERE pseudo = :pseudo');
								$req_user->execute(array(
									':pseudo' => $_SESSION['pseudo'],
								));
								$pseudo_credential = $req_user->fetch();

								$pseudo_cre = $pseudo_credential['pseudo'];
