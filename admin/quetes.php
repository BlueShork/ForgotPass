...
<?php
	...
	<html>
		...
		<body>
            ...
            <li class="active"><a href="quetes.php"><i class="fab fa-elementor logo"></i><span class="br"> Vérifications</span></a>
            ...
            <li><a href="inscription_verif.php"><i class="fas fa-list logo"></i><span class="br"> Inscriptions</span></a></li>
	...
	padding-right: 15px;">+ POST</a></li>
        ...
        <?php
			...
			$videosParPage = 100;
			$videosTotalesReq = $bdd->query('SELECT * FROM quetes_liens');
			...
			?>
				...
				$reponse = $bdd->query('SELECT COUNT(*) AS counte FROM quetes_verif');
                $donnees = $reponse->fetch();
                echo $donnees['counte'];
				?>).</h1>
				</div> 
...
<h1>Participations quêtes totale pour <?php echo $_GET['pseudo']; ?> : (<?php 
				
				
				
				$reponse = $bdd->prepare('SELECT COUNT(*) AS counteee FROM quetes_liens WHERE pseudo = :pseudo');
				$reponse->execute(array(
					':pseudo' => $_GET['pseudo'],
				));
                $donnees = $reponse->fetch();
                echo $donnees['counteee'];
				?>).</h1>
<div class="signalements quetes perso">
        	<?php 
				
				
				
				$reponse = $bdd->prepare('SELECT COUNT(*) AS countee FROM quetes_liens WHERE pseudo = :pseudo');
				$reponse->execute(array(
					':pseudo' => $_GET['pseudo'],
				));
                $donnees = $reponse->fetch();
				...
				?>
        	<h3><span class="box"><?php  echo $donnees['countee']; ?></span> Quêtes<span class="right-i"><i class="fas fa-caret-up"></i></span></h3>
			<?php
				$req = $bdd->prepare('SELECT * FROM quetes_liens WHERE pseudo = :pseudo ORDER BY id DESC');
				$req->execute(array(
					':pseudo' => $_GET['pseudo'],
				));
				?>
				...
				while($donnees = $req->fetch()){
					...
					<div class="card">
						...
						?></p>
							...
							<p class="id inline"><?php echo $donnees['email']; ?></p>
							<p class="id inline"><?php echo $donnees['dates_quetes']; ?></p>
								<p class="paiement ciffreinline no_border"><span class="chiffre coins"><?php echo $donnees['gains']; ?> <i class="fas fa-bullseye"></i></span></p>
								<div class="suppression"><p class="supprimer inline"><a href="supprimer_part_quetes.php?id=<?php echo $donnees['id'];?>&pseudo=<?php echo $donnees['pseudo'];?>&dates_quetes=<?php echo $donnees['dates_quetes'];?>"><i class="fas fa-times"></i></a></p></div>
					</div>
					<?php
				}
...
<div class="signalements quetes">
			...
			<?php
				$req = $bdd->query('SELECT * FROM quetes_verif ORDER BY id DESC LIMIT  '.$depart.','.$videosParPage);
				?>
				<?php
				while($donnees = $req->fetch()){
					$id = $donnees['id'];
...
<div class="signalements quetes ptc">
        	<?php 
				...
				$reponse = $bdd->query('SELECT COUNT(*) AS counte FROM ptc_verif');
                $donnees = $reponse->fetch();
                
				?>
        	<h3><span class="box"><?php  echo $donnees['counte']; ?></span> PTC<span class="right-i"><i class="fas fa-caret-up"></i></span></h3>
			<?php
				$req = $bdd->query('SELECT * FROM ptc_verif ORDER BY id DESC LIMIT  '.$depart.','.$videosParPage);
				?>
				<?php
				while($donnees = $req->fetch()){
					$id = $donnees['id'];
					...
					<div class="card">
						...
						?></p>
							...
							<p class="id inline"><?php echo $donnees['email']; ?></p>
								<p class="paiement ciffreinline no_border"><span class="chiffre coins"><?php echo $donnees['gains']; ?> <i class="fas fa-bullseye"></i></span></p>
								<div class="suppression"><p class="supprimer inline"><a href="supprimer_part_ptc.php?id=<?php echo $donnees['id'];?>&pseudo=<?php echo $donnees['pseudo'];?>&dates_quetes=<?php echo $donnees['dates_quetes'];?>"><i class="fas fa-times"></i></a></p></div>
					</div>
					<?php
				}
